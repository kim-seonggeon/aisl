...
- Visual SLAM 기반으로 돌리게 되면 내가 원하는 건 ORB-SLAM 이지만 설치 때 의존성 문제가 많다고 해서 Visual SLAM 기반으로 돌려본 경험이 있는 랩원에게 OpenVINS 프로젝트를 추천받아 우선 이것부터 SLAM 테스트해보기로 결정(https://github.com/rpng/open_vins <- 참고)
